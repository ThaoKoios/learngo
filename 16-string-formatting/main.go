...
func main() {
	...
	// If the values is a struct, the %+v variant will include the struct's field names
	fmt.Printf("%+v\n", p)

	// The %#v variant prints a Go syntax representation of the value,
	// i.e. the source code snippet that would product that value
	fmt.Printf("%#v\n", p)
	...
	// There are many options for formatting interger.
	// Use %d for standard, base-10 formatting
	fmt.Printf("%d\n", 123)

	// This %b prints a binary representation
	fmt.Printf("%b\n", 14)

	// This %c prints the character corresponding to the given integer
	fmt.Printf("%c\n", 33)
	...
	// To specify the width of an integer, use a number after the % inte verb
	// By default the result will be right-justified and padded with spaces.
	fmt.Printf("|%6d|%6d|\n", 12, 345)

	// You can also specify the width of printed floats, though usuallly you will also want to restrict the decimal precision
	// at the same time with the width.precision syntax.
	fmt.Printf("|%6.2f|%6.2f|\n", 1.2, 3.45)

	// To left justify, use the '-' flags
	fmt.Printf("|%-6.2f|%-6.2f|\n", 1.2, 3.45)

	// You may also want to control width when formatting strings,
	// especially to ensure that they align in tabel-like output. For basic right-justified width
	fmt.Printf("|%6s|%6s|\n", "foo", "b")

	// To left-justify use the '-' flags as with numbers
	fmt.Printf("|%-6s|%-6s|\n", "foo", "b")

	// So far we have seen Printf, which prints the formatted string to 'os.Stdout.Sprintf'
