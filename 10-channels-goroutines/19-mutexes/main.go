...
func main() {
	...
	// We will also start 10 goroutines to simulate writes, using the same pattern we did for reads
	for w := 0; w < 10; w++ {
		go func() {
			for {
				key := rand.Intn(5)
...
// Running the program shows that we executed about 90000 total operations against our 'mutex0synchronized state'
// We will look at implementing this same state management task using only goroutines and channels
