/*
In the mutexes example we used explicit locking with mutexes to synchronize access to shared state across multiple goroutines.
Another option is to use the built-in synchronization features of goroutines and channels to achieve the same result.package main
This channel-based approach aligns with Go's ideas of sharing memory by communicating ang having each piece of data ownned by exactly one goroutine.
*/
...
// Running our program shows that the goroutine-based state management example completes about 80000 total operations

/*
For this particular case the goroutine-based approach was a bit more involved tha nthe mutex-based one.
