// Go offers built-in support for regular expressions. Here are some examples of common regexp-related task in Go
...
func main() {
	...
	// Above we used a string pattern directly, but for other 'regexp' tasks you will need to 'Compile' an optimized 'Regexp' struct
	r, _ := regexp.Compile("p([a-z]+)ch")
	// Many methods are available on these structs. Here is a match test like we saw earlier
	...
	// A plain Complie won't work for constants because it has 2 return values.
	r = regexp.MustCompile("p([a-z]+)ch")
	fmt.Println(r)

	// The regexp package can alse be used to replace subsets of strings with other values
	fmt.Println(r.ReplaceAllString("a peach", "<fruit>"))

	// Th e Func variant allows you to transform matched text with a given function
	in := []byte("a peach")
	out := r.ReplaceAllFunc(in, bytes.ToUpper)
	fmt.Println(string(out))
}

// For a complete reference on Go regular expressions check the regexp package docs (https://golang.org/pkg/regexp/)
